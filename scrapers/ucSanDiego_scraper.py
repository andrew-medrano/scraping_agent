...
def cleanup():
    """Ensures gRPC connections from Gemini API shut down properly before exit."""
    print("Gemini API shutdown successfully.")
...
class ScraperConfig:
    start_urls: List[str] = field(default_factory=lambda: [
        "https://techtransfer.universityofcalifornia.edu/Default?campus=SD&RunSearch=True&ShowDetail=False&SortASC=False&SortColumn=NCDId"
    ])
    university: str = "ucSanDiego"  # FILL THIS OUT: This is the name of the university
    ...
    selectors = {
        'item_links': ".table-row .tech-link",
        'next_button': ".next"
    }
    jina_remove_selectors = '.ncd-sub-right-panel, #subBlockFour, .table-container-fluid, .sticky-menu-bar, #myNavbar, .small-fixed-header'
    jina_target_selectors = '.ncd-main-right-panel'
...
class ContentExtractor:
    ...
    def _parse_llm_response(self, content: str) -> Dict[str, str]:
        """Parses Gemini response and ensures missing values are blank instead of null."""
        try:
            # Remove triple backticks if Gemini returns a JSON code block
            if content.startswith("```json"):
                content = "\n".join(content.split("\n")[1:-1])
            
            parsed_data = json.loads(content)

            # Ensure all fields are strings and replace None/null with ""
            expected_fields = ["ip_name", "ip_number", "published_date", "ip_description", "patents"]
            cleaned_data = {field: (parsed_data.get(field, "") or "").strip() for field in expected_fields}

            return cleaned_data

        except json.JSONDecodeError as e:
            print(f"\nERROR: Failed to parse Gemini response as JSON: {str(e)}\nContent received:\n{content}")
            return {field: "" for field in ["ip_name", "ip_number", "published_date", "ip_description", "patents"]}
            ...
            while True:
                ...
                if not detail_urls:
                    ...
                    if self.config.parallel:
                        ...
                        with ThreadPoolExecutor(max_workers=2) as executor:  # Limits concurrent API calls
                            futures = [
